...
## Todo2（待完成項目）
1. 回合制：5戰3勝（Option: 3,5,～12）✓ 已完成
2. 統計戰績（左玩家：1 平手：2 右玩家：2）✓ 已完成
3. 開場隨機配置玩家座位（左/右邊）和符號（O/X）✓ 已完成
4. 開場左玩家先手，下一回合輪替 ✓ 已完成
5. Protocol 以座標方式交換（井字棋盤座標）✓ 已完成
6. UI 相同符號連線時，需要即時畫出連線線條 ✓ 已完成
7. game.py 類別規畫，不夠完善 ✓ 已完成（移除未使用的枚舉和方法）
8. 沒有定義前、後端溝通的 protocol，日後難維護 ✓ 已完成（見 PROTOCOL.md）
9. game.py 沒有驗證自我功能的正確性，除錯困難 ✓ 已完成（30個單元測試全通過）
10. game 有 WIN_CONDITIONS，卻沒有提供哪幾條線連線（狀態）✓ 已完成
11. 從 app 到 game 的流程太迂迴（包太多層相同介面），效率不佳 → 持續優化中
12. 每次都把 board 全部的資都往 view 丟（資料過度複雜）✓ 已優化（只傳單格更新）
13. 少考慮 2 條連線情況 → 待評估
14. app 沒有類別化 ✓ 已完成（WebApp 類別）
15. AIPlayer 與一般玩家沒有制定相同的介面與功能 ✓ 已移除 AI 功能
16. 登入有 bug: 選定圖示，按登入報錯「無此圖示」✓ 已修正
